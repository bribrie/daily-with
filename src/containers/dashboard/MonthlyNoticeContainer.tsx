...
const MonthlyNoticeContainer = () => {
  ...
  const [itemCount, showAddForm, resetItemCountList] =
    useItemCount("공지사항을");
  const [dragChagnedList, setDragChagnedList] = useState<NoticeListType[]>([]);
  const [showSavedButton, setShowSavedButton] = useState(false);
  const dispatch = useAppDispatch();
  const loading = useAppSelector(noticeLoading);

  useEffect(() => {
    if (list.length === 0 && loading === "idle") {
      try {
    ...
    if (dragChagnedList.length !== list.length) {
      setDragChagnedList(list);
    }
  }, [list, loading, dragChagnedList.length, userUid, dispatch]);
